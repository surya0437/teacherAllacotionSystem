...
if (strlen($_SESSION['tsasaid'] == 0)) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $select = mysqli_query($con, "SELECT * FROM `tblcourse` WHERE `CourseName` = '$cname'");

        if (mysqli_num_rows($select) > 0) {
            echo '<script>alert("Faculty is already exist. Please add another faculty");
            window.location.href ="course.php"</script>';
        } else {
            $sql = "insert into tblcourse(CourseName)values(:coursename)";
            // $sql = "insert into tblcourse(FacultyName,CourseName)values(:facultyname,:coursename)";
            $query = $dbh->prepare($sql);
            // $query->bindParam(':facultyname', $bname, PDO::PARAM_STR);
...
?>
    ...
    <head>
        ...
        <title>TSAS : Faculty Create</title>
                            ...
                            <div class="col-md-4">
                                <div class="card alert">
                                    <div class="card-header pr">
                                        <h4>Create A New Faculty</h4>
                                        <form method="post" name="hjhgh">
                                            <div class="basic-form m-t-20">
                                                <div class="form-group">
                                                    <label>Faculty Name
                                                    </label>
                                                    <input type="text" class="form-control border-none input-flat bg-ash" placeholder="Faculty Name" name="coursename" required="true">
                                                </div>
                                            </div>
                                            <!-- <div class="basic-form m-t-20">
                                                <div class="form-group">
                                                    <label>Faculty Name</label>
                                                    <input type="text" class="form-control border-none input-flat bg-ash" placeholder="Faculty Name" name="facultyname" required="true">
                                                </div>
                                            </div> -->
